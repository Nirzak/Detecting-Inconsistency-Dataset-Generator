...
def export_df_to_dataset(df, output_dir):
    df["body"] = df.apply(lambda row: [bnltk.sentence_tokenize(para) for para in row["body"]], axis=1)

    def tokenize_sent_in_para(para):
        result = []
        for sent in para:
            result.append(" ".join(bnltk.word_tokenize(sent)))
        result = " <EOS> ".join(result) + " <EOS> "
        return result
    df["body"] = df.apply(lambda row: [tokenize_sent_in_para(para) for para in row["body"]], axis=1)

    df["headline"] = df.apply(lambda row: " ".join(bnltk.word_tokenize(row["headline"])), axis=1)
...
def export_df_to_dataset_para(df, output_dir):
    df["fake_paras"] = df.apply(lambda row: [bnltk.sentence_tokenize(para) for para in row["fake_paras"]], axis=1)
    df["base"] = df.apply(lambda row: [bnltk.sentence_tokenize(para) for para in row["base"]], axis=1)
